...
export function SkillsSection({ skills, skillDetails }: SkillsSectionProps) {
        ...
        {inView && details && (
          <div className="mt-3 p-6 rounded-xl border-2 border-slate-200 border-solid bg-white text-slate-700 w-full max-w-[640px] shadow-[0_10px_30px_rgba(0,0,0,0.08)] reveal reveal--visible">
            <div className="flex gap-4 items-center mb-4">
              ...
              <h3 className="m-0 text-xl font-semibold text-slate-900">{details.title}</h3>
            </div>
            <p className="mb-3 text-base font-medium text-slate-600">{details.description}</p>
            <ul className="p-0 m-0">
              {details.details.map((detail, detailIndex) => (
                <li className="relative pl-4 mb-2 text-sm text-slate-600" key={detailIndex}>{detail}</li>
              ))}
            </ul>
          </div>
        )}
  ...
  return (
    <section className="mb-8">
      <h2 className="pb-2 mb-6 text-3xl font-semibold text-slate-900 border-solid border-b-[3px] border-b-slate-300">Fähigkeiten</h2>
